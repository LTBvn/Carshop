...
function RegisterPage() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    login({ id: 'user-02', name, email, role: 'customer' }, 'token-02');
    navigate('/dashboard');
  };
        ...
        <div className="w-full max-w-md space-y-8">
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Đăng ký để trở thành khách hàng thân thiết của TA Car
            </p>
          </div>
          ...
          <p className="text-center text-sm text-slate-600 dark:text-slate-400">
            Đã có tài khoản?{' '}
            <Link to="/login" className="font-semibold text-slate-950 dark:text-white hover:text-violet-600 dark:hover:text-violet-400">
              Đăng nhập ngay
            </Link>
          </p>
